...
- [x] N2.asmdef: `AgeOfArena.asmdef` (refs: Unity.Postprocessing.Runtime) + `AgeOfArenaEditor.asmdef` (Editor-only, refs AgeOfArena) + `AgeOfArenaTests.asmdef` (EditMode, refs AgeOfArena + NUnit). `Assets/Tests/SelfTests.cs` — 10 test: CombatMath.SelfTest/NetDamage×2, FP +/-/*/Sqrt/Distance, GridPathfinder.SelfTest, FPMath NetDamageInt/InRangeSq×2. 0 error / 0 warning; isCompilationSuccessful=true.
...
- [x] N2.mapgen: `MapGenerator.cs` pure static sınıf — MonoBehaviour/GameObject yok; `MapType` enum + `Archetype` struct (displayName/coastInner/coastOuter/coastClusters/basePositions/groveCenters/groveRadius/extras/contestedMines/forceNomad). N10 RMS ile birlikte yapıldı. 0/0.
...
- [x] N3.fixedstep: `GameManager.FixedStepEnabled` + `_stepAccumulator` — 30Hz sabit-adım döngüsü (`while acc >= FIXED_DT { SimTick(FIXED_DT); acc -= FIXED_DT }`); maks 3 adım/frame (spiral-of-death koruması); opt-in flag (SP=false, lockstep=true). `SimTick(dt)` ayrı metoda çıkarıldı. Render interpolasyon (N16.lockstep ile birlikte). 0/0.
- [x] N3.cmdlog: `CommandRecorder.cs` MonoBehaviour — `CommandType` enum (Move/AttackMove/Attack/Gather/Build/Garrison/Patrol/SetStance/SetRally/Train/Research/Delete/Stop/Ping) + `GameCommand` [Serializable] struct (tick/playerId/type/unitIds/intParam1+2/x+z/floatParam1+2). CommandSystem.MoveOrder/Attack/Gather, TrainingQueue.Enqueue, ResearchSystem.Enqueue kayıt çağrıları eklendi. GameManager.cmdRecorder; WorldRoot AddComponent. Snapshot()/LoadSnapshot()/ForTick(). 0/0.
...
- [x] N8.siege: `UnitFactory`: Trebuchet/Ram/Mangonel `KenneyModels.Spawn("Castle/siege-trebuchet/ram/catapult", t, Vector3.zero, 1.1f)` ile Kenney FBX; primitive fallback korundu; `Prims.Box` takım-rengi pennant/banner eklendi. `UnitEntity.demolishedPrefab` alanı + `Die()` → `Instantiate(demolishedPrefab)` + `Destroy(wreck, 5f)` (yıkık variant ölümde 5s sonra kaybolur). 4 naval birim (Galley/FireShip/DemoShip/FishingShip) zaten primitive gövde mesh'e sahip. 0 error / 0 warning. Runtime doğrulaması gerekiyor (Kenney siege modelleri Play'de görünür; ölümde yıkık variant spawn edilir).
...
### Wave 3 — SP-Derinlik
- [x] N10.rms: `MapGenerator.Get(MapType)` → 5 arketip (Arena/Arabistan/KaraOrman/Adalar/Göçebe) seed-deterministik (aynı `mapSeed`+`mapType` = aynı yerleşim). `WorldRoot.mapType` alanı + `_arch` ile BasePositions/coastForest/interiorClumps/baseResources/contestedMines arketip verisinden. `GameBootstrap.NextMapType`; `CivSelectScreen` "Harita" döngü butonu (Map|Diff|Mode|Start sırasına eklendi). Nomad arketip `forceNomad=true` → `GameMode.Nomad` zorlanır. Runtime doğrulaması: harita butonu dönüşümü + Arabia/BlackForest/Islands'ın farklı kaynak yerleşimi Play'de görünmeli. 0/0.
...
- [x] N11.trig: `TriggerSystem.cs` MonoBehaviour — `ConditionType` (Timer/OwnUnits/OwnBuildings/ResourceGathered/EnemyEliminated/TechResearched/AgeReached) + `EffectType` (YouWin/YouLose/ShowMessage/ShowObjective/AddResource/ActivateTrigger/DeactivateTrigger/SetGameOver/PlaySound) + `TriggerData` [Serializable] struct. GameManager.triggers; WorldRoot SetupGameManager wires it; GatherSystem.OnResourceDeposited callback. SaveSystem v4: triggers serialize/restore in F5/F9 + ApplyPendingLoad. SP版: effects apply directly (command-log + fixed-step → N3/N15'e ertelendi). 0 error / 0 warning.
- [x] N12.edit: `ScenarioEditor.cs` runtime overlay Canvas (sortingOrder 6000) — sol palette (Birimler/Binalar/Kaynaklar kategorileri), takım seçici, yerleştirme (zemin raycast→UnitFactory/BuildingFactory/ResourceFactory), silme (Physics.Raycast→GetComponentInParent), per-player kaynak/çağ ayarı. Save/Load PlayerPrefs JSON (SaveData v4); "Test Et" → PendingLoad + Restart. HUD pause menüsüne "📝 Editör" butonu; 'E' tuşu ile açılır, Esc ile kapanır. GameManager.scenarioEditor; WorldRoot SetupGameManager'da AddComponent. 0 error / 0 warning.
...
- [x] N13.aow: `ArtOfWarSystem.cs` — 4 challenge (EarlyEconomy/Combat/Counters/Siege), her biri bronze/silver/gold derecelendirmeli trigger seti. `ArtOfWarChallenge` enum; `ArtOfWarSystem.Setup(gm)` WorldRoot SetupGameplay sonunda çağrılır; CivSelectScreen "Savaş Sanatı" döngü butonu + aktif/kapalı etiketi. GatherSystem → TriggerSystem.OnResourceDeposited ResourceGathered koşulunu besler. TechResearched/AgeReached/EnemyEliminated/OwnUnits koşulları farklı challenge'larda kullanılıyor. 0 error / 0 warning.
- [x] N13.camp: `CampaignSystem.cs` static — 3 görev (İlk Savaş/Kaynak Savaşı/İmparatorun Seferi), her biri TriggerData seti, başlangıç kaynakları, Castle/Imperial Age koşulu. PlayerPrefs per-mission progress (unlocked/done). `CampaignScreen.cs` Canvas overlay — 3 mission card (kilitli/hazır/tamamlandı), ▶ Başla → CampaignSystem.ActiveMissionId + Restart(). HUD.ShowGameOver → CampaignSystem.OnCampaignWin() kazanınca ilerlemeyi kaydeder + sonrakini açar. HUD pause menüsüne "⚔ Kampanya" butonu. WorldRoot.SetupGameplay sonunda CampaignSystem.Setup(gm) çağrılır. 0 error / 0 warning.
...
### Wave 4 — Multiplayer
- [x] N15.spike: **Karar kaydı: custom lockstep seçildi.** Photon Quantum spike yapılamadı (SDK kurulumu + WebGL test ortamı gerektiriyor; lisans sınırlamaları var). Bunun yerine custom yol tercih edildi: (1) Mevcut MonoBehaviour sim tek-thread sıralı tick — lockstep'e zaten uygun. (2) SimRandom (N3.prng) seed determinizmi hazır. (3) NavMesh'ten bağımsız grid A*/flow-field (N16.path) daha kontrollü. (4) WebSocket relay WebGL kısıtını aşar (N17). Deterministik pathfinding yaklaşımı: grid A* seçildi (flow-field büyük haritalar için; SP haritası küçük → A* yeterli, N16.path'e ertelendi). Per-tick state checksum + command recorder → N15.checksum + N3.cmdlog.
- [x] N15.checksum: `ChecksumSystem.cs` MonoBehaviour — FNV-32 hash (unit pos/hp/state + building hp + team resources, her 30 frame); HISTORY_SIZE=512 entry queue. SaveReplaySnapshot() → PlayerPrefs JSON (seed+commands+checksums). StartReplayVerify() → GameBootstrap.ReplayBaseline seti + Restart(seed). LoadBaseline() WorldRoot.Build sonunda çağrılır; tick-by-tick CompareBaseline() PASS/MISMATCH@tick=N sonucu. GameBootstrap.ReplayBaseline; HUD pause menüsüne "🔁 Replay" butonu. GameManager.checksum; WorldRoot AddComponent. 0/0.
- [x] N16.fixed: `FixedPoint.cs` — `FP` Q16.16 struct (long raw; 16bit integer + 16bit fraction; range ±32767, precision ~0.000015). Ops: +/-/*/÷/Abs/Max/Min/Clamp/Sqrt (Newton-Raphson integer). `FPVec2` (x,z normalized/magnitude/+/-/*). `FPMath` (NetDamageInt deterministik, InRangeSq no-Sqrt threshold). `FP.Distance(ax,az,bx,bz)` tam integer. Render: ToFloat() → Transform her frame. Cross-platform: yalnız int/long aritmetik, IEEE-754 float yok. 0/0.
- [x] N16.path: `GridPathfinder.cs` — uniform XZ grid (CELL=1.5u, SIZE=~134×134 over ±100u); bool[] passable; InitFromRadius disc; SetBlocked(worldPos). A* octile heuristic, integer costs (straight=10/diag=14), SortedList open-set, no-float in search loop. `FindPath(Vector2 start, goal, out List<Vector2> waypoints)`. `SelfTest()` open-disc path doğrulaması. WorldRoot BakeNavMesh sonrası `GridPathfinder.Build(LandRadius)` çağrısı. SP=NavMesh, MP=GridPathfinder (NavMesh-free). 0/0.
- [x] N16.lockstep: `LockstepSystem.cs` MonoBehaviour — INPUT_DELAY=2 frame, per-player Dictionary<tick,Queue<GameCommand>> kuyrukları; `EnqueueLocal/EnqueueRemote/MirrorToP1`. Solo-mirror modu (PlayerCount=1): P0 komutları P1'e kopyalanır. `OnSimTick()` GameManager.SimTick() sonunda çağrılır; transport.SendChecksum ile checksum yayınlanır. `StartLockstep()` GameManager.FixedStepEnabled=true seti. GameManager.lockstep ref; WorldRoot AddComponent. 0/0.
- [x] N17.transport: `TransportLayer.cs` MonoBehaviour — `TransportMode` (Loopback/WebSocket); `Connect/Disconnect/SendCommand/SendChecksum/SendLobbySetup`; `OnCommandReceived/OnChecksumReceived/OnConnected/OnDisconnected` events. Loopback: self-echo (yerel test). WebSocket stub: TODO işaretli (NativeWebSocket/Cloudflare Worker); lobby handshake (seed+civ+difficulty+mapType). WorldRoot OnChecksumReceived → DesyncHandler.CheckTick wiring. 0/0.
- [x] N17.desync: `DesyncHandler.cs` MonoBehaviour — `CheckTick(tick, localHash, remoteHash)`: uyumsuzlukta `DesyncReport` string, HUD subtitle, `Time.timeScale=0`, `checksum.SaveReplaySnapshot()` state dump. `Reset()`. GameManager.desync ref; WorldRoot AddComponent. 0/0.
- [x] N17.replay: `ReplayViewer.cs` — `StartViewer/StopViewer`; bottom-bar Canvas (Tick sayaç, hız ×0.5/1/2/4, ↺ yeniden, ✕). Update(): tick label + PASS/FAIL renk; PASS'ta `timeScale=0`. `TryAutoStart()` WorldRoot.Build() sonunda (replay key varsa). Forward-only playback (rewind = restart replay). ChecksumSystem.ReplayVerifyResult ile entegre. 0/0.
