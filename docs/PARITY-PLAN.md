...
- [x] FISH: UnitType.FishingShip (naval, atk 0, ArmorClass.Ship, hp60) Dock'tan (Dark) eğitilir; CommandSystem balıkçıya Food node toplatır; GatherSystem ile FishPond/FishTrap'ten gather → Dock deposit (Dock MaskFood). BuildingType.FishTrap (50 odun) Start'ta co-located FishPond node kaydeder. Runtime: ship naval/hp60/atk0, gather assign (kind=Food, state=Moving), FishTrap→FishPond(Food/250), Dock food kabul doğrulandı.
